...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!dest) return { title: 'Cafe tidak ditemukan' }
  return {
    title: `${dest.name} — Cafe & Tempat Nongkrong Surabaya | MLAKOOW`,
    description: dest.description.slice(0, 160),
  }
}

export default async function DetailCafePage({
  params,
}: {
            ...
            >
              {[
                ...
                },
                {
                  ...
                  value: averageMenuPrice === 0 ? 'Bervariasi' : (
                    averageMenuPrice < 30000 ? '$ (Murah)' :
                    averageMenuPrice < 60000 ? '$$ (Sedang)' :
                    averageMenuPrice < 100000 ? '$$$ (Mahal)' : '$$$$ (Premium)'
                  ),
                ...
                },
                {
                  icon: Wifi,
                  label: 'WiFi & WFC',
                  value: destination.facilities?.includes('WiFi') ? 'Tersedia' : 'Tidak Ada',
                  color: '#0284C7',
                },
                {
                  icon: MapPin,
