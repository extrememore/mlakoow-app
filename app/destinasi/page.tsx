...
export default function DestinasiPage() {
  ...
  useEffect(() => {
    ...
    // Load categories
    fetch('/api/destinations?limit=50')
      .then((r) => r.json())
      .then((d) => {
        const cats = new Map()
      ...
      >
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', fontWeight: 900, marginBottom: '0.75rem' }}>
            Semua Destinasi Surabaya
          </h1>
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '1rem', maxWidth: '560px' }}>
            Temukan {total > 0 ? `${total}+` : ''} destinasi wisata terbaik di Kota Surabaya — dari landmark populer hingga hidden gems
          </p>
        </div>
      </div>
